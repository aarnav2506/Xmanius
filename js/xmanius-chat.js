(() => {
  ...
  const ask = async (question) => { const q = question.trim(); if (!q || activeRequestController) return; addMessage(q, "user"); input.value = ""; const local = localAnswer(q); if (local && !thinkMode) { addMessage(local, "assistant", { animate: true }); return; } const thinking = document.createElement("article"); thinking.className = "message assistant thinking ai-message--thinking"; thinking.setAttribute("role", "status"); thinking.innerHTML = `<span>${thinkMode ? "Thinking carefully" : "Thinking"}</span><i></i><i></i><i></i>`; list.append(thinking); document.querySelector(".chat-content").scrollTop = document.querySelector(".chat-content").scrollHeight; activeRequestController = new AbortController(); setSendingState(true); try { const response = await fetch("/api/xmanius-chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: q, thinkMode }), signal: activeRequestController.signal }); const data = await response.json(); thinking.remove(); addMessage(response.ok ? data.reply : (data.error || `The AI request failed (${response.status}).`), "assistant", { animate: true }); } catch (error) { thinking.remove(); if (error.name === "AbortError") addMessage("The response was stopped by you.", "assistant", { animate: true }); else addMessage(`The AI service could not be reached. ${error?.message || "Please check the deployment and API configuration."}`, "assistant", { animate: true }); } finally { activeRequestController = null; setSendingState(false); } };
