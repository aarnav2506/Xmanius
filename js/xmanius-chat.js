(() => {
  ...
  const addMessage = (text, type, { animate = false, persist = true, sources = [], searchError = "" } = {}) => {
    ...
    if (animate && type === "assistant") { item.classList.add("message-entering"); body.querySelectorAll(":scope > *").forEach((node, index) => node.style.setProperty("--line-delay", `${index * 42}ms`)); window.setTimeout(() => item.classList.remove("message-entering"), 3500); }
  ...
  const ask = async (question) => { const q = question.trim(); if (!q || activeRequestController) return; if (!updateUsage()) return; const history = conversationHistory(); addMessage(q, "user"); input.value = ""; updateUsage(true); const local = localAnswer(q); if (local && !thinkMode && !webSearch && selectedModel === "xmanius-1") { addMessage(local, "assistant", { animate: true }); return; } const thinking = document.createElement("article"); thinking.className = "message assistant thinking ai-message--thinking"; thinking.setAttribute("role", "status"); thinking.innerHTML = `<span>${thinkMode ? "Thinking carefully" : webSearch ? "Searching multiple sources" : "Thinking"}</span><i></i><i></i><i></i><span class="xmanius-live-cursor" aria-hidden="true"></span>`; list.append(thinking); document.querySelector(".chat-content").scrollTop = document.querySelector(".chat-content").scrollHeight; activeRequestController = new AbortController(); const startedAt = performance.now(); const timeout = window.setTimeout(() => activeRequestController?.abort(), 120000); setSendingState(true); try { const response = await fetch("/api/xmanius-chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: q, model: selectedModel, thinkMode, webSearch, history }), signal: activeRequestController.signal }); const data = await response.json().catch(() => ({})); thinkingSeconds = Math.max(1, Math.round((performance.now() - startedAt) / 1000)); thinking.remove(); addMessage(response.ok ? data.reply : (data.userMessage || data.error || `The AI request failed (${response.status}).`), "assistant", { animate: true, sources: response.ok ? data.sources : [], searchError: response.ok ? data.searchError : "" }); } catch (error) { thinking.remove(); if (error.name === "AbortError") addMessage("The response was stopped by you.", "assistant", { animate: true }); else addMessage(`The AI service could not be reached. ${error?.message || "Please check the deployment and API configuration."}`, "assistant", { animate: true }); } finally { window.clearTimeout(timeout); activeRequestController = null; setSendingState(false); } };
