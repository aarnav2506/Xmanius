...
(() => {
  ...
  function getUserProfile() {
    ...
    const emailPrefix = state.user.email ? state.user.email.split("@")[0] : "user";
    const displayName = customName || meta.full_name || meta.name || identityMeta.full_name || identityMeta.name || emailPrefix;

    let autoGeneratedUsername = localStorage.getItem("xmanius-auto-username");
    if (!autoGeneratedUsername) {
      const cleanBase = (displayName.toLowerCase().replace(/[^a-z0-9]/g, "") || "user").slice(0, 8);
      const randomSuffix = Math.random().toString(36).substring(2, 7);
      autoGeneratedUsername = `${cleanBase}_${randomSuffix}`;
      localStorage.setItem("xmanius-auto-username", autoGeneratedUsername);
    }

    const username = (customUsername || meta.username || meta.user_name || identityMeta.user_name || autoGeneratedUsername).replace(/^@/, "");
    const email = state.user.email || "";
    const avatarUrl = customAvatar || meta.avatar_url || meta.picture || identityMeta.avatar_url || identityMeta.picture || "";

    return { displayName, username, email, avatarUrl, isGuest: false };
  }
  ...
  function updateUI() {
    ...
    if (accountStatus) {
      if (profile.isGuest) {
        accountStatus.textContent = "Log In / Sign Up";
      } else {
    ...
    if (avatarSpan) {
      if (profile.isGuest) {
        avatarSpan.innerHTML = `<svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor" aria-hidden="true"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>`;
